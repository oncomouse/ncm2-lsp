...
class Source(Ncm2Source):
    ...
    def on_complete(self, context):
        ...
        if self.vim.call('has', 'nvim-0.5.0'):
            ...
            if context['base'] == prev_input and self.vim.vars[
                    'ncm2_lsp#_requested']:
                candidates = self.process_candidates()
            else:
                vars = self.vim.vars
                vars['ncm2_lsp#_requested'] = False
                vars['ncm2_lsp#_prev_input'] = context['base']
                vars['ncm2_lsp#_complete_position'] = context['startccol']

                params = self.vim.call('luaeval',
                                       'vim.lsp.util.make_position_params()')

                self.vim.call(
                    'luaeval', 'require("candidates").request_candidates('
                    '_A.arguments)', {'arguments': params})
    ...
    def process_candidates(self):
        candidates = []
        vars = self.vim.vars
        results = vars['ncm2_lsp#_results']
        if not results:
            return []
        ...
        for rec in items:
            if 'textEdit' in rec and rec['textEdit'] is not None:
                ...
                if textEdit['range']['start'] == textEdit['range']['end']:
                    previous_input = vars['ncm2_lsp#_prev_input']
                    complete_position = vars['ncm2_lsp#_complete_position']
            ...
            elif (isinstance(rec.get('documentation'), dict)
